...
# Estimand: risk ratio with Delta_C = 1.54 and Delta_I = 1.18
# Methods: unadjusted, GEE, LMM, Aug-GEE, TMLE, Eff-PM, Eff-ML
# The population size for each cluster is 10 or 50, which is different from simulation-2-2.
# ------------
rm(list = ls())
set.seed(123)
library(tidyverse)
...
var_rr <- function(IF, mu){  c(1/mu[2], -mu[1]/mu[2]^2) %*% var(IF) %*% c(1/mu[2], -mu[1]/mu[2]^2) }
...
m<- 10000
pi <- 0.5
N <- sample(c(10,50), size = m, replace = T)
C1 <- rnorm(m, mean = 0.1 * N, sd = 2)
C2 <- rbinom(m, size = 1, prob = expit(C1 * log(N/10)))
A <- rbinom(m, size = 1, prob = pi)
complete_data <- map(1:m, function(j) {
  X1 <- rbinom(N[j], size = 1, prob = N[j]/50)
  X2 <- rnorm(N[j], mean = ifelse(C2[j], 1, -1) * mean(X1), sd = 3)
  # M1 <- 10 + sample(c(-1,0), size = 1)
  # M0 <- 10 + sample(c(-1,0), size = 1)
  M1 <- ifelse(N[j] > 49, 10, 2) + 5 * C2[j]
  M0 <- ifelse(N[j] > 49, 6, 3)
  Y1 <- rbinom(N[j], size = 1, prob = expit(sin(C1[j]) * (2 *C2[j] - 1) * N[j]/30 +   1.5 * exp(X1) * sqrt(abs(X2)) - N[j]/20))
  Y0 <- rbinom(N[j], size = 1, prob = expit(sin(C1[j]) * (2 *C2[j] - 1) * N[j]/30 +   1.5 * (2 *X1 - 1)  * sqrt(abs(X2)) + rnorm(N[j], sd = 1)))
  cluster_id <- j
...
print(paste0("simulation2-1-m",m,"-",simulation_scenario))
...
saveRDS(results, paste0("simulation2-1-m",m,"-",simulation_scenario, ".rds"))
stopCluster(cl)
