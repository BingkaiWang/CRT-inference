...
# Estimand: average treatment effect with Delta_C = 6 and Delta_I = 8.6667
# Methods: unadjusted, GEE, LMM, Aug-GEE, TMLE, Eff-PM, Eff-ML
# The population size for each cluster is 10 or 50, which is different from simulation-1-2.
# ------------
rm(list = ls())
set.seed(123)
library(tidyverse)
...
print(paste0("simulation1-1-m",m,"-",simulation_scenario))
...
results <- foreach(iter = 1:sim_size, .combine = cbind, .packages = package_list) %dopar% {
  ...
  complete_data <- map(1:m, function(j) {
    ...
    Y1 <- sin(C1[j]) * (2 *C2[j] - 1) * N[j]/30 + 5 * exp(X1) * abs(X2) + rnorm(N[j], sd = 1) + N[j]/5
    Y0 <- sin(C1[j]) * (2 *C2[j] - 1) * N[j]/30 + 5 * exp(X1) * abs(X2) + rnorm(N[j], sd = 1) + rnorm(1, mean = 0, sd = 1)
    cluster_id <- j
...
saveRDS(results, paste0("simulation1-1-m",m,"-",simulation_scenario,".rds"))
